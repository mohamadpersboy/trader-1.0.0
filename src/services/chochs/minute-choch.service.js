...
export async function detectMinuteChoch(fromIndex = 0) {
...
export async function chockDetector(candle, primary, secondary) {
    ...
    if (primary.type === "bearish" && primary.base === null) {
        ...
        const previousCandle = await MinuteCandle.findOne({
...
async function createBearishSpecialChoch(candle, primary) {
    ...
    const lastChoch = await MinuteChoch
        .findOne()
        .sort({index: -1});
...
async function createChochFromPrimary(primary) {
    ...
    return await MinuteChoch.create(chochData);
}
...
async function resetChoch(id, type) {
    ...
    try {
        ...
        return await TempMinuteChoch.findByIdAndUpdate(id, {
            type,
...
export async function getOrCreateTempChoch() {
    ...
    let primary = await TempMinuteChoch.findOne({
        name: "primary",
    });
    ...
    if (!primary) {
        ...
        primary = await TempMinuteChoch.create({
    ...
    let secondary = await TempMinuteChoch.findOne({
        name: "secondary",
    });
    ...
    if (!secondary) {
        ...
        secondary = await TempMinuteChoch.create({
